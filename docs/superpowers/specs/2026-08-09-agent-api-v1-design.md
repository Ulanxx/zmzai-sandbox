...
Relay adds `POST /api/internal/sandbox/models`. It requires the existing Sandbox service authorization, accepts `{ "sandboxKey": "zsk_..." }`, and resolves the key exactly once to derive its owner. It returns `{ "models": [...] }`, filtered to enabled and routable public models. Relay returns `401` for an invalid service secret or key, and Sandbox maps an unavailable or malformed Relay response to `503 RELAY_UNAVAILABLE`. This endpoint must not forward the key to Relay's public `/api/v1/models` route or return account, channel, or provider fields.
...
The status enum is `queued`, `planning`, `running`, `cancellation_requested`, `succeeded`, `failed`, or `cancelled`. Terminal states are `succeeded`, `failed`, and `cancelled`. A failed run includes `{ "failure": { "code": "...", "error": "...", "retryable": false } }`; `ownerSandboxKeyId`, provider ids, and execution leases are never returned to API clients.
...
- The provider callback persists `providerSandboxId` immediately after instance creation, before executing the command. On startup, a recovery worker atomically claims only expired leases, retries deletion for their recorded provider id, releases capacity, appends an orphan-recovery event, and marks the run failed. A worker with a current lease is never touched by recovery.
- Cancellation atomically transitions an active run to `cancellation_requested` and returns `202` with that snapshot. The executor or recovery worker aborts the command and deletes the recorded instance. Successful cleanup transitions to `cancelled`; a failed cleanup transitions to `failed` with `SANDBOX_CLEANUP_FAILED` and remains on the recovery retry queue. A natural completion may transition from `running` only; if cancellation won the compare-and-set race, it cannot overwrite the cancellation outcome. Repeating cancellation returns `202` while cleanup is pending and `200` for a terminal run.
...
4. A restarted Sandbox process replays a final run's persisted events, and safely recovers an expired active lease without touching a worker with a renewed lease.
5. A revoked key cannot read or cancel its former runs; a different valid key receives `404`.
6. Relay balance rejection, concurrency exhaustion, an invalid model, cancellation races, failed provider cleanup, and OpenSandbox failure all return the documented transport/error or terminal-run behavior.
